...
pipeline {
    ...
    environment {
        ...
        GITHUB_REPO_NAME = 'k8s-yaml-manifests'
    ...
    stages {
        ...
        stage('YAML Lint') {
            steps {
                script {
                    echo 'Running YAML Limit'
                    try {
                        def result = sh(script: 'yamllint .', returnStatus: true)
                        if (result != 0) {
                            updateGitHubStatus('lint-validate', 'failure', 'Lint Validate check failed', env.PR_COMMIT_SHA)
                            error('Lint validate check failed!')
                        }
                        echo "Lint validate succeeded. Updating GitHub status to success."
                        updateGitHubStatus('lint-validate', 'success', 'Lint Validate check passed', env.PR_COMMIT_SHA)
                    } catch (Exception e) {
                        echo "Lint validate failed: ${e.message}"
